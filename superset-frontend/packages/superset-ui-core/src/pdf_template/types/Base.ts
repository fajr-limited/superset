...
export enum Behavior {
  InteractivePdfTemplate = 'INTERACTIVE_PDF_TEMPLATE',
...
export enum PdfTemplateLabel {
  Deprecated = 'DEPRECATED',
  Featured = 'FEATURED',
}

export const chartLabelExplanations: Record<PdfTemplateLabel, string> = {
  [PdfTemplateLabel.Deprecated]:
    'This chart uses features or modules which are no longer actively maintained. It will eventually be replaced or removed.',
  [PdfTemplateLabel.Featured]:
    'This chart was tested and verified, so the overall experience should be stable.',
};

export const chartLabelWeight: Record<PdfTemplateLabel, { weight: number }> = {
  [PdfTemplateLabel.Deprecated]: {
    weight: -0.1,
  },
  [PdfTemplateLabel.Featured]: {
