...
import { Behavior, PdfTemplateLabel } from '../types/Base';
...
export interface PdfTemplateMetadataConfig {
  ...
  // deprecated: true hides a pdf_template from all viz picker interactions.
  deprecated?: boolean;
  // label: PdfTemplateLabel.DEPRECATED which will display a "deprecated" label on the pdf_template.
  label?: PdfTemplateLabel | null;
  labelExplanation?: string | null;
  queryObjectCount?: number;
  parseMethod?: ParseMethod;
  // suppressContextMenu: true hides the default context menu for the pdf_template.
  // This is useful for viz plugins that define their own context menu.
  suppressContextMenu?: boolean;
}

export default class PdfTemplateMetadata {
  name: string;
  ...
  constructor(config: PdfTemplateMetadataConfig) {
    const {
      name,
  ...
  clone() {
    return new PdfTemplateMetadata(this);
  }
}
