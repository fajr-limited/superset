...
import PdfTemplateClient, { SliceIdAndOrFormData } from '../clients/PdfTemplateClient';
...
export type PdfTemplateDataProviderProps =
...
export type PdfTemplateDataProviderState = {
  status: 'uninitialized' | 'loading' | 'error' | 'loaded';
  payload?: ProvidedProps['payload'];
  error?: ProvidedProps['error'];
};

class PdfTemplateDataProvider extends PureComponent<
  PdfTemplateDataProviderProps,
  PdfTemplateDataProviderState
> {
  readonly pdf_templateClient: PdfTemplateClient;

  constructor(props: PdfTemplateDataProviderProps) {
    super(props);
    this.state = { status: 'uninitialized' };
    this.pdf_templateClient = new PdfTemplateClient({ client: props.client });
  }
  ...
  componentDidUpdate(prevProps: PdfTemplateDataProviderProps) {
      ...
      try {
        this.pdf_templateClient
          ...
          .then(formData =>
            Promise.all([
              loadDatasource
                ? this.pdf_templateClient.loadDatasource(
                    formData.datasource,
                    datasourceRequestOptions,
                  )
                : Promise.resolve(undefined),
              this.pdf_templateClient.loadQueryData(formData, queryRequestOptions),
            ]).then(
...
export default PdfTemplateDataProvider;
