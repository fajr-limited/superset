...
from superset.commands.pdf_template.exceptions import PdfTemplateNotFoundError
...
class ExportPdfTemplatesCommand(ExportModelsCommand):
    dao = PdfTemplateDAO
    not_found = PdfTemplateNotFoundError

    @staticmethod
    def _file_name(model: PdfTemplate) -> str:
        file_name = get_filename(model.slice_name, model.id)
        return f"pdf_templates/{file_name}.yaml"

    @staticmethod
    def _file_content(model: PdfTemplate) -> str:
    ...
    @staticmethod
    def _export(
        model: PdfTemplate, export_related: bool = True
    ) -> Iterator[tuple[str, Callable[[], str]]]:
        yield (
            ExportPdfTemplatesCommand._file_name(model),
            lambda: ExportPdfTemplatesCommand._file_content(model),
        )
