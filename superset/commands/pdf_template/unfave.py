...
class DelFavoritePdfTemplateCommand(BaseCommand):
    def __init__(self, chart_id: int) -> None:
        self._chart_id = chart_id
        self._chart: Optional[PdfTemplate] = None

    @transaction(on_error=partial(on_error, reraise=PdfTemplateUnfaveError))
    def run(self) -> None:
        self.validate()
        return PdfTemplateDAO.remove_favorite(self._chart)

    def validate(self) -> None:
        chart = PdfTemplateDAO.find_by_id(self._chart_id)
        if not chart:
